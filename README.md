...
Alternatively, download the [fwup executable](https://github.com/fwup-home/fwup/releases/download/v1.16.0/fwup.exe)
and place it in your path.

If you're using another platform or prefer to build it yourself, download the
latest [source code release](https://github.com/fwup-home/fwup/releases/download/v1.16.0/fwup-1.16.0.tar.gz)
...
ubi_volume_write(device_path)           | 1.17.0 | Write a resource atomically to a UBI volume (e.g. `/dev/ubi0_3`). Requires the `--unsafe` flag. Linux-only — fwup must be built with mtd-utils headers (`<mtd/ubi-user.h>`).
