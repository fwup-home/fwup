...
static int maybe_mount(struct block_cache *block_cache, off_t block_offset, size_t block_count, TCHAR *mount_path)
{
    ...
    // Find a cached mount slot.
    for (i = 0; i < FF_VOLUMES; i++) {
        if (mounted_[i].output == NULL)
            break;
    ...
    mount_path[1] = ':';
    mount_path[2] = 0;
    CHECK("fat_mount", NULL, f_mount(&mounted_[i].fs, mount_path, 0));

    // On success, fill out the rest.
    mounted_[i].output = block_cache;
    mounted_[i].block_offset = block_offset;
    mounted_[i].block_count = block_count;
...
int fatfs_mkdir(struct block_cache *output, off_t block_offset, const char *dir)
{
    ...
    TCHAR absolute_dir[FF_LFN_BUF + 1];
    snprintf(absolute_dir, sizeof(absolute_dir), "%s\\%s", mount_path, dir);
...
int fatfs_setlabel(struct block_cache *output, off_t block_offset, const char *label)
{
    ...
    TCHAR absolute_label[FF_LFN_BUF + 1];
...
int fatfs_rm(struct block_cache *output, off_t block_offset, const char *cmd, const char *filename, bool file_must_exist)
{
    ...
    TCHAR absolute_filename[FF_LFN_BUF + 1];
    snprintf(absolute_filename, sizeof(absolute_filename), "%s\\%s", mount_path, filename);
...
int fatfs_mv(struct block_cache *output, off_t block_offset, const char *cmd, const char *from_name, const char *to_name, bool force)
{
    ...
    TCHAR absolute_from_name[FF_LFN_BUF + 1];
    snprintf(absolute_from_name, sizeof(absolute_from_name), "%s\\%s", mount_path, from_name);
    TCHAR absolute_to_name[FF_LFN_BUF + 1];
    snprintf(absolute_to_name, sizeof(absolute_to_name), "%s\\%s", mount_path, to_name);
