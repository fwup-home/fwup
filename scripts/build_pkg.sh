...
FWUP_INSTALL_DIR=$BUILD_DIR/fwup-staging/$CROSS_COMPILE/usr
...
pushd fwup
LDFLAGS="-L$DEPS_INSTALL_DIR/lib $EXTRA_LDFLAGS" CPPFLAGS=-I$DEPS_INSTALL_DIR/include ./configure $CONFIGURE_ARGS --prefix=$FWUP_INSTALL_DIR --enable-shared=no || cat config.log
cat config.log
make clean
make $MAKE_FLAGS
